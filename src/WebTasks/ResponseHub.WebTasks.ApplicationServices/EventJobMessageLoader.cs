	...
	public class EventJobMessageLoader
	{
		...
		public async Task SetJobMessagesForActiveEvents()
		{
			...
			foreach(Event activeEvent in activeEvents)
			{
				...
				IList<Guid> jobMessageIds = await _jobMessageService.GetMessageIdsBetweenDates(unitCapcodes.Select(i => i.CapcodeAddress), MessageType.Job, activeEvent.StartDate, DateTime.UtcNow.AddHours(1));
